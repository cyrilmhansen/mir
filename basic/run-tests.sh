...
run_tests() {
        ...
        run_test() {
                local name="$1"
                local in_file src exp
                if [[ "$name" == test/* ]]; then
                        local base="${name#test/}"
                        in_file="$ROOT/basic/test/$base.in"
                        src="$ROOT/basic/test/$base.bas"
                        exp="$ROOT/basic/test/$base.out"
                else
                        in_file="$ROOT/basic/samples/$name.in"
                        src="$ROOT/basic/samples/$name.bas"
                        exp="$ROOT/basic/samples/$name.out"
                fi
...
PY
        ...
        diff "$ROOT/basic/samples/hcolor_test.out" "$ROOT/basic/hcolor_test.out"
        echo "hcolor_test OK"
        ...
        if [[ "$BASICC" == *-ld ]]; then
                cc -Wall -Wextra -DBASIC_USE_LONG_DOUBLE -I"$ROOT/basic/include" -I"$ROOT/basic/src" \
                        ...
                        "$ROOT/basic/src/basic_pool_test.c" \
        ...
        diff "$ROOT/basic/samples/basic_pool_test.out" "$ROOT/basic/basic_pool_test.out"
        ...
        cc -Wall -Wextra -I"$ROOT/basic/include" -I"$ROOT/basic/src" -I"$ROOT" -ffunction-sections \
                ...
                "$ROOT/basic/src/basic_runtime_lowmem_test.c" \
        ...
        diff "$ROOT/basic/samples/basic_runtime_lowmem_test.out" "$ROOT/basic/basic_runtime_lowmem_test.out"
        ...
        if [[ "$BASICC" == *-ld ]]; then
                cc -shared -fPIC -Wall -Wextra -DBASIC_USE_LONG_DOUBLE -I"$ROOT/basic/include" -I"$ROOT/basic/src" \
                        "$ROOT/basic/src/extlib.c" -o "$ROOT/basic/libextlib.so"
        else
