...
static const Builtin builtins[]
  = {{"RND", 0, 0},    {"INT", 0, 0},       {"TIMER", 0, 0},   {"TIME", 1, 0},
     ...
     {"ASINH", 0, 0},  {"ACOSH", 0, 0},     {"ATANH", 0, 0},   {"LOG", 0, 0},
     {"LOG2", 0, 0},   {"LOG10", 0, 0},     {"EXP", 0, 0},     {"FACT", 0, 0},
     {"PI", 0, 0},     {"LEN", 0, 1},       {"VAL", 0, 0},     {"ASC", 0, 1},
     {"INSTR", 0, 1},  {"MIRCTX", 0, 1},    {"MIRMOD", 0, 1},  {"MIRFUNC", 0, 1},
     {"MIRREG", 0, 1}, {"MIRLABEL", 0, 1},  {"MIREMIT", 0, 1}, {"MIREMITLBL", 0, 1},
     {"MIRRET", 0, 1}, {"MIRFINISH", 0, 1}, {"MIRRUN", 0, 1},  {"MIRDUMP", 0, 1},
     {"CHR$", 1, 0},   {"UNICHAR$", 1, 0},  {"STRING$", 1, 0}, {"TIME$", 1, 0},
     {"DATE$", 1, 0},  {"INPUT$", 1, 0},    {"SPC", 1, 0},     {"SPACE$", 1, 0},
     {"TAB", 0, 0},    {"LEFT$", 1, 0},     {"RIGHT$", 1, 0},  {"MID$", 1, 0},
     {"STR$", 1, 0},   {"INKEY$", 1, 0},    {"MIRROR$", 1, 0}, {"UPPER$", 1, 0},
     {"LOWER$", 1, 0}, {NULL, 0, 0}};
...
static Node *parse_literal (Parser *p) {
  ...
  case TOK_NUMBER:
    ...
    n = new_node (N_NUM);
    n->num = t.num;
    if (basic_num_eq (t.num, basic_num_from_int (basic_num_to_int (t.num)))) n->is_int = 1;
    return n;
  case TOK_STRING:
    t = next_token (p);
    n = new_node (N_STR);
...
static Node *parse_not (Parser *p) {
  ...
  if (n->left != NULL) n->is_int = n->left->is_int;
  return n;
}
...
static Node *parse_add (Parser *p) {
  ...
  while (1) {
    ...
    if (!nn->is_str) nn->is_int = n->is_int && r->is_int;
    n = nn;
  }
  return n;
}
...
static int parse_decimal_stmt (Parser *p MIR_UNUSED, Stmt *out) {
  if (decimal_locked) {
...
static MIR_reg_t gen_unop (MIR_context_t ctx, MIR_item_t func, VarVec *vars, Node *n) {
...
static MIR_reg_t gen_binop (MIR_context_t ctx, MIR_item_t func, VarVec *vars, Node *n) {
  if (n->is_str) {
  ...
  if (n->op == OP_AND) {
    ...
    basic_num_hooks.mir_bcmp (ctx, func, MIR_DBEQ, MIR_new_label_op (ctx, false_lab),
                              MIR_new_reg_op (ctx, l), emit_num_const (ctx, BASIC_ZERO));
    MIR_reg_t r = gen_expr (ctx, func, vars, n->right);
    basic_num_hooks.mir_bcmp (ctx, func, MIR_DBEQ, MIR_new_label_op (ctx, false_lab),
                              MIR_new_reg_op (ctx, r), emit_num_const (ctx, BASIC_ZERO));
    MIR_append_insn (ctx, func,
  ...
  MIR_reg_t r = gen_expr (ctx, func, vars, n->right);
  char buf[32];
  if (n->op == OP_BIT || n->op == OP_XOR || n->op == OP_NAND || n->op == OP_NOR || n->op == OP_SHL
      || n->op == OP_SHR) {
    safe_snprintf (buf, sizeof (buf), "$t%d", tmp_id++);
    MIR_reg_t li = MIR_new_func_reg (ctx, func->u.func, MIR_T_I64, buf);
    basic_num_hooks.mir_n2i (ctx, func, MIR_new_reg_op (ctx, li), MIR_new_reg_op (ctx, l));
    safe_snprintf (buf, sizeof (buf), "$t%d", tmp_id++);
    MIR_reg_t ri = MIR_new_func_reg (ctx, func->u.func, MIR_T_I64, buf);
    basic_num_hooks.mir_n2i (ctx, func, MIR_new_reg_op (ctx, ri), MIR_new_reg_op (ctx, r));
    safe_snprintf (buf, sizeof (buf), "$t%d", tmp_id++);
    MIR_reg_t resi = MIR_new_func_reg (ctx, func->u.func, MIR_T_I64, buf);
    if (n->op == OP_SHL || n->op == OP_SHR) {
      MIR_insn_code_t sop = n->op == OP_SHL ? MIR_LSH : MIR_URSH;
      MIR_append_insn (ctx, func,
                       MIR_new_insn (ctx, sop, MIR_new_reg_op (ctx, resi), MIR_new_reg_op (ctx, li),
    ...
    if (n->left->is_str || n->right->is_str) {
      ...
      basic_num_hooks.mir_binop (ctx, func, cmp_code, MIR_new_reg_op (ctx, resi),
    ...
    if (n->op == OP_BACKSLASH || n->op == OP_MOD) {
      ...
      MIR_append_insn (ctx, func,
                       MIR_new_insn (ctx, iop, MIR_new_reg_op (ctx, resi), MIR_new_reg_op (ctx, li),
                                     MIR_new_reg_op (ctx, ri)));
      basic_num_hooks.mir_i2n (ctx, func, MIR_new_reg_op (ctx, res), MIR_new_reg_op (ctx, resi));
    } else if (n->op == OP_POW) {
      MIR_append_insn (ctx, func,
      ...
      basic_num_hooks.mir_binop (ctx, func, op, MIR_new_reg_op (ctx, res), MIR_new_reg_op (ctx, l),
                                 MIR_new_reg_op (ctx, r));
    }
    return res;
  }
  ...
  if (strcasecmp (n->var, "RND") == 0) {
    ...
    MIR_append_insn (ctx, func,
                     MIR_new_call_insn (ctx, 4, MIR_new_ref_op (ctx, int_proto),
                                        MIR_new_ref_op (ctx, int_import), MIR_new_reg_op (ctx, res),
                                        MIR_new_reg_op (ctx, arg)));
  }
  return res;
}

static MIR_reg_t gen_str (MIR_context_t ctx, MIR_item_t func, Node *n) {
  char buf[32];
  safe_snprintf (buf, sizeof (buf), "$t%d", tmp_id++);
  MIR_reg_t r = MIR_new_func_reg (ctx, func->u.func, MIR_T_I64, buf);
  MIR_append_insn (ctx, func,
                   MIR_new_insn (ctx, MIR_MOV, MIR_new_reg_op (ctx, r),
                                 MIR_new_str_op (ctx, (MIR_str_t) {strlen (n->str) + 1, n->str})));
  return r;
}
...
static MIR_reg_t gen_var (MIR_context_t ctx, MIR_item_t func, VarVec *vars, Node *n) {
  if (n->index != NULL) {
    MIR_reg_t base = get_array (vars, ctx, func, n->var, 0, 0, n->is_str);
    ensure_array_dim (vars, ctx, func, n->var, base, n->is_str, n->index2 != NULL);
    MIR_reg_t idxd1 = gen_expr (ctx, func, vars, n->index);
    char buf[32];
    safe_snprintf (buf, sizeof (buf), "$t%d", tmp_id++);
    MIR_reg_t idx = MIR_new_func_reg (ctx, func->u.func, MIR_T_I64, buf);
    basic_num_hooks.mir_n2i (ctx, func, MIR_new_reg_op (ctx, idx), MIR_new_reg_op (ctx, idxd1));
    if (array_base != 0)
      MIR_append_insn (ctx, func,
                       MIR_new_insn (ctx, MIR_SUB, MIR_new_reg_op (ctx, idx),
                                     MIR_new_reg_op (ctx, idx), MIR_new_int_op (ctx, array_base)));
    if (n->index2 != NULL) {
      MIR_reg_t idxd2 = gen_expr (ctx, func, vars, n->index2);
      safe_snprintf (buf, sizeof (buf), "$t%d", tmp_id++);
      MIR_reg_t idx2 = MIR_new_func_reg (ctx, func->u.func, MIR_T_I64, buf);
      basic_num_hooks.mir_n2i (ctx, func, MIR_new_reg_op (ctx, idx2), MIR_new_reg_op (ctx, idxd2));
      if (array_base != 0)
        MIR_append_insn (ctx, func,
    ...
    MIR_append_insn (ctx, func,
                     MIR_new_insn (ctx, MIR_MUL, MIR_new_reg_op (ctx, off),
                                   ...
                                   MIR_new_int_op (ctx, n->is_str ? 8 : sizeof (basic_num_t))));
    safe_snprintf (buf, sizeof (buf), "$t%d", tmp_id++);
    ...
    MIR_append_insn (ctx, func,
                     MIR_new_insn (ctx, n->is_str ? MIR_MOV : BASIC_MIR_MOV,
                                   ...
                                   MIR_new_mem_op (ctx, n->is_str ? MIR_T_P : BASIC_MIR_NUM_T, 0,
                                                   addr, 0, 1)));
...
static MIR_reg_t gen_expr (MIR_context_t ctx, MIR_item_t func, VarVec *vars, Node *n) {
  ...
  switch (n->kind) {
  case N_STR: return gen_str (ctx, func, n);
  case N_NUM: return gen_num (ctx, func, n);
  case N_VAR: return gen_var (ctx, func, vars, n);
  default: break;
  }
  if (n->is_str) {
    if (n->kind == N_CALL) {
      ...
      if (strcasecmp (n->var, "CHR$") == 0) {
        ...
        basic_num_hooks.mir_n2i (ctx, func, MIR_new_reg_op (ctx, argi), MIR_new_reg_op (ctx, arg));
        MIR_append_insn (ctx, func,
        ...
        basic_num_hooks.mir_n2i (ctx, func, MIR_new_reg_op (ctx, a1i), MIR_new_reg_op (ctx, a1));
        ...
        basic_num_hooks.mir_n2i (ctx, func, MIR_new_reg_op (ctx, cnti), MIR_new_reg_op (ctx, cnt));
        MIR_append_insn (ctx, func,
        ...
        basic_num_hooks.mir_n2i (ctx, func, MIR_new_reg_op (ctx, starti),
        ...
        if (n->index != NULL) {
          ...
          basic_num_hooks.mir_n2i (ctx, func, MIR_new_reg_op (ctx, leni),
                                   MIR_new_reg_op (ctx, len));
      ...
      return res;
    } else if (n->kind == N_BIN) {
      return gen_binop (ctx, func, vars, n);
    }
    return 0;
  }
  if (n->kind == N_NEG || n->kind == N_NOT) {
    return gen_unop (ctx, func, vars, n);
  } else if (n->kind == N_CALL) {
    if (strcasecmp (n->var, "RND") == 0 || strcasecmp (n->var, "INT") == 0)
    ...
    if (strcasecmp (n->var, "TIMER") == 0) {
      ...
      if (n->left->is_int) {
        ...
        MIR_reg_t resi = MIR_new_func_reg (ctx, func->u.func, MIR_T_I64, buf);
        MIR_append_insn (ctx, func,
                         MIR_new_call_insn (ctx, 4, MIR_new_ref_op (ctx, iabs_proto),
                                            MIR_new_ref_op (ctx, iabs_import),
                                            MIR_new_reg_op (ctx, resi),
                                            MIR_new_reg_op (ctx, argi)));
        basic_num_hooks.mir_i2n (ctx, func, MIR_new_reg_op (ctx, res), MIR_new_reg_op (ctx, resi));
      } else {
        MIR_append_insn (ctx, func,
      ...
      if (n->left->is_int) {
        ...
        MIR_reg_t resi = MIR_new_func_reg (ctx, func->u.func, MIR_T_I64, buf);
        MIR_append_insn (ctx, func,
                         MIR_new_call_insn (ctx, 4, MIR_new_ref_op (ctx, isgn_proto),
                                            MIR_new_ref_op (ctx, isgn_import),
                                            MIR_new_reg_op (ctx, resi),
                                            MIR_new_reg_op (ctx, argi)));
        basic_num_hooks.mir_i2n (ctx, func, MIR_new_reg_op (ctx, res), MIR_new_reg_op (ctx, resi));
      } else {
      ...
      MIR_append_insn (ctx, func,
                       MIR_new_call_insn (ctx, 4, MIR_new_ref_op (ctx, len_proto),
                                          ...
                                          MIR_new_reg_op (ctx, resi), MIR_new_reg_op (ctx, arg)));
      basic_num_hooks.mir_i2n (ctx, func, MIR_new_reg_op (ctx, res), MIR_new_reg_op (ctx, resi));
      ...
      MIR_append_insn (ctx, func,
                       MIR_new_call_insn (ctx, 5, MIR_new_ref_op (ctx, instr_proto),
                                          ...
                                          MIR_new_reg_op (ctx, resi), MIR_new_reg_op (ctx, s),
...
static void print_item (Node *e, Node *next) {
  if (e->kind == N_TAB) {
    ...
    basic_num_hooks.mir_n2i (g_ctx, g_func, MIR_new_reg_op (g_ctx, ni), MIR_new_reg_op (g_ctx, n));
    MIR_append_insn (g_ctx, g_func,
...
static void gen_print_hash (Stmt *s) {
  ...
  basic_num_hooks.mir_n2i (g_ctx, g_func, MIR_new_reg_op (g_ctx, fni), MIR_new_reg_op (g_ctx, fn));
...
static void gen_input (Stmt *s) {
  ...
  for (size_t i = 0; i < s->u.input.n; i++) {
    ...
    if (v->index == NULL) {
      ...
      basic_num_hooks.mir_n2i (g_ctx, g_func, MIR_new_reg_op (g_ctx, idx),
      ...
      if (v->index2 != NULL) {
        ...
        basic_num_hooks.mir_n2i (g_ctx, g_func, MIR_new_reg_op (g_ctx, idx2),
...
static void gen_poke (Stmt *s) {
  ...
  basic_num_hooks.mir_n2i (g_ctx, g_func, MIR_new_reg_op (g_ctx, addri),
                           MIR_new_reg_op (g_ctx, addr));
  safe_snprintf (buf, sizeof (buf), "$t%d", tmp_id++);
  MIR_reg_t vali = MIR_new_func_reg (g_ctx, g_func->u.func, MIR_T_I64, buf);
  basic_num_hooks.mir_n2i (g_ctx, g_func, MIR_new_reg_op (g_ctx, vali),
                           MIR_new_reg_op (g_ctx, val));
...
static void gen_stmt_for (Stmt *s) {
  ...
  basic_num_hooks.mir_bcmp (g_ctx, g_func, MIR_DBLT, MIR_new_label_op (g_ctx, neg_step),
                            MIR_new_reg_op (g_ctx, step), emit_num_const (g_ctx, BASIC_ZERO));
  basic_num_hooks.mir_bcmp (g_ctx, g_func, MIR_DBGT, MIR_new_label_op (g_ctx, end_label),
  ...
  basic_num_hooks.mir_bcmp (g_ctx, g_func, MIR_DBLT, MIR_new_label_op (g_ctx, end_label),
                            MIR_new_reg_op (g_ctx, var), MIR_new_reg_op (g_ctx, end));
...
// 
  ...
  // ... (many other prototypes remain unchanged) ...
...
*/
  ...
  system_proto = BASIC_PROTO_GEN(ctx, "basic_system_p", 0, NULL, 2, 
...
#ifdef BASIC_USE_FIXED64
  randomize_proto = BASIC_PROTO_GEN (ctx, "basic_randomize_p", 0, NULL, 2,
                                     {.type = BASIC_MIR_NUM_T, .name = "n"},
                                     {.type = BASIC_MIR_NUM_T, .name = "has_seed"});
#else
...
#ifdef BASIC_USE_FIXED64
  get_line_proto = BASIC_PROTO_GEN (ctx, "basic_get_line_p", 0, NULL, 1,
                                    {.type = MIR_T_P, .name = "res"});
#else
...
// --- START OF REPLACEMENT BLOCK ---
  ...
  // ... (iabs_proto and isgn_proto are correct) ...
  ...
  // ... (Apply the same #ifdef pattern for sin, cos, tan, atn, sinh, cosh, tanh, etc.) ...
  // ... (asinh, acosh, atanh, asin, acos, log, log2, log10, exp, fact) ...
...
// FIXED GEMINI
#ifdef BASIC_USE_FIXED64
  val_proto = BASIC_PROTO_GEN(ctx, "basic_val_p", 0, NULL, 2, 
                              {.type = MIR_T_P, .name = "res", .size = 0}, 
                              {.type = MIR_T_P, .name = "s", .size = 0});
#else
...
#ifdef BASIC_USE_FIXED64
  mir_ctx_proto = BASIC_PROTO_GEN (ctx, "basic_mir_ctx_p", 0, NULL, 1, {.type = MIR_T_P, .name = "res"});
#else
  mir_ctx_proto = BASIC_PROTO_NUM (ctx, "basic_mir_ctx_p", 0);
#endif
  mir_ctx_import = MIR_new_import (ctx, "basic_mir_ctx");
...
#ifdef BASIC_USE_FIXED64
  mir_mod_proto = BASIC_PROTO_GEN (ctx, "basic_mir_mod_p", 0, NULL, 3, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "ctx_h", .size = 0}, {.type = MIR_T_P, .name = "name", .size = 0});
#else
  mir_mod_proto = BASIC_PROTO_NUM (ctx, "basic_mir_mod_p", 2, {BASIC_MIR_NUM_T, "ctx_h", 0}, {MIR_T_P, "name", 0});
#endif
  mir_mod_import = MIR_new_import (ctx, "basic_mir_mod");

#ifdef BASIC_USE_FIXED64
  mir_func_proto = BASIC_PROTO_GEN (ctx, "basic_mir_func_p", 0, NULL, 4, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "mod_h", .size = 0}, {.type = MIR_T_P, .name = "name", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "nargs", .size = 0});
#else
  mir_func_proto = BASIC_PROTO_NUM (ctx, "basic_mir_func_p", 3, {BASIC_MIR_NUM_T, "mod_h", 0}, {MIR_T_P, "name", 0}, {BASIC_MIR_NUM_T, "nargs", 0});
#endif
  mir_func_import = MIR_new_import (ctx, "basic_mir_func");

#ifdef BASIC_USE_FIXED64
  mir_reg_proto = BASIC_PROTO_GEN (ctx, "basic_mir_reg_p", 0, NULL, 2, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "func_h", .size = 0});
#else
  mir_reg_proto = BASIC_PROTO_NUM (ctx, "basic_mir_reg_p", 1, {BASIC_MIR_NUM_T, "func_h", 0});
#endif
  mir_reg_import = MIR_new_import (ctx, "basic_mir_reg");

#ifdef BASIC_USE_FIXED64
  mir_label_proto = BASIC_PROTO_GEN (ctx, "basic_mir_label_p", 0, NULL, 2, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "func_h", .size = 0});
#else
  mir_label_proto = BASIC_PROTO_NUM (ctx, "basic_mir_label_p", 1, {BASIC_MIR_NUM_T, "func_h", 0});
#endif
  mir_label_import = MIR_new_import (ctx, "basic_mir_label");

#ifdef BASIC_USE_FIXED64
  mir_emit_proto = BASIC_PROTO_GEN (ctx, "basic_mir_emit_p", 0, NULL, 6, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "func_h", .size = 0}, {.type = MIR_T_P, .name = "op", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "a", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "b", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "c", .size = 0});
#else
  mir_emit_proto = BASIC_PROTO_NUM (ctx, "basic_mir_emit_p", 5, {BASIC_MIR_NUM_T, "func_h", 0}, {MIR_T_P, "op", 0}, {BASIC_MIR_NUM_T, "a", 0}, {BASIC_MIR_NUM_T, "b", 0}, {BASIC_MIR_NUM_T, "c", 0});
#endif
  mir_emit_import = MIR_new_import (ctx, "basic_mir_emit");

#ifdef BASIC_USE_FIXED64
  mir_emitlbl_proto = BASIC_PROTO_GEN (ctx, "basic_mir_emitlbl_p", 0, NULL, 3, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "func_h", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "lab_h", .size = 0});
#else
  mir_emitlbl_proto = BASIC_PROTO_NUM (ctx, "basic_mir_emitlbl_p", 2, {BASIC_MIR_NUM_T, "func_h", 0}, {BASIC_MIR_NUM_T, "lab_h", 0});
#endif
  mir_emitlbl_import = MIR_new_import (ctx, "basic_mir_emitlbl");

#ifdef BASIC_USE_FIXED64
  mir_ret_proto = BASIC_PROTO_GEN (ctx, "basic_mir_ret_p", 0, NULL, 3, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "func_h", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "reg_h", .size = 0});
#else
  mir_ret_proto = BASIC_PROTO_NUM (ctx, "basic_mir_ret_p", 2, {BASIC_MIR_NUM_T, "func_h", 0}, {BASIC_MIR_NUM_T, "reg_h", 0});
#endif
  mir_ret_import = MIR_new_import (ctx, "basic_mir_ret");

#ifdef BASIC_USE_FIXED64
  mir_finish_proto = BASIC_PROTO_GEN (ctx, "basic_mir_finish_p", 0, NULL, 2, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "mod_h", .size = 0});
#else
  mir_finish_proto = BASIC_PROTO_NUM (ctx, "basic_mir_finish_p", 1, {BASIC_MIR_NUM_T, "mod_h", 0});
#endif
  mir_finish_import = MIR_new_import (ctx, "basic_mir_finish");

#ifdef BASIC_USE_FIXED64
  mir_run_proto = BASIC_PROTO_GEN (ctx, "basic_mir_run_p", 0, NULL, 6, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "func_h", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "a1", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "a2", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "a3", .size = 0}, {.type = BASIC_MIR_NUM_T, .name = "a4", .size = 0});
#else
  mir_run_proto = BASIC_PROTO_NUM (ctx, "basic_mir_run_p", 5, {BASIC_MIR_NUM_T, "func_h", 0}, {BASIC_MIR_NUM_T, "a1", 0}, {BASIC_MIR_NUM_T, "a2", 0}, {BASIC_MIR_NUM_T, "a3", 0}, {BASIC_MIR_NUM_T, "a4", 0});
#endif
  mir_run_import = MIR_new_import (ctx, "basic_mir_run");

#ifdef BASIC_USE_FIXED64
  mir_dump_proto = BASIC_PROTO_GEN (ctx, "basic_mir_dump_p", 0, NULL, 2, {.type = MIR_T_P, .name = "res"}, {.type = BASIC_MIR_NUM_T, .name = "func_h", .size = 0});
#else
  mir_dump_proto = BASIC_PROTO_NUM (ctx, "basic_mir_dump_p", 1, {BASIC_MIR_NUM_T, "func_h", 0});
#endif
  mir_dump_import = MIR_new_import (ctx, "basic_mir_dump");
...
#if defined(BASIC_USE_LONG_DOUBLE)
  basic_num_mode_init (BASIC_NUM_MODE_LONG_DOUBLE);
#elif defined(BASIC_USE_MSFP)
  basic_num_mode_init (BASIC_NUM_MODE_MSFP);
#else
  basic_num_mode_init (BASIC_NUM_MODE_DOUBLE);
#endif
